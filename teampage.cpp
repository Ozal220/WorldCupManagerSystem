...
//    model = new myModel(this);
    ...
    model->select(); //查询整张表
    model->setHeaderData(0, Qt::Horizontal, tr("球队名")); //更改列名 // 多表操作用 model->setQuery(sql 语句);
    model->setHeaderData(1, Qt::Horizontal, tr("已赛场次"));
    model->setHeaderData(2, Qt::Horizontal, tr("赢"));
    model->setHeaderData(3, Qt::Horizontal, tr("平"));
    model->setHeaderData(4, Qt::Horizontal, tr("输"));
    model->setHeaderData(5, Qt::Horizontal, tr("净胜球"));
    model->setHeaderData(6, Qt::Horizontal, tr("进球"));
    model->setHeaderData(7, Qt::Horizontal, tr("积分"));
    model->setHeaderData(8, Qt::Horizontal, tr("分组"));
